...
if((sizeof($result)) < 1){
	...
	$stmt = $conn->query("call get_usr_lunch_meals('$userID')");
	if ($stmt) {
		$query = $stmt ->fetchall(PDO::FETCH_ASSOC);
		if (sizeof($query)<1){
			?><p>You haven't added any lunch meals yet :( Time to start experimenting!</p><?php
		}else{
			?>
			<div class="card-group">
			<?php foreach ($query as $meal): ?>
				<div class="card"><img style = "max-height:300px;" class="card-img-top w-100 d-block" src=<?= $meal['meal_image']; ?>>
					<div class="card-body">
		...
		?>
		<p>You haven't added any lunch meals yet :( Time to start experimenting!</p>
		<?php 
	}
}
